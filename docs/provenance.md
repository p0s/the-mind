...
Goal: keep every reader-facing claim traceable to a *source id + locator*, while keeping prose readable and keeping the input syntax machine-checkable.
...
- `locator` is one of:
  - timecode: `HH:MM:SS` or `HH:MM:SS.mmm` (use `00:00:00` for non-timecoded written sources when there is no better locator)
  - PDF pages: `pN` or `pN-M` (1-based; maps to `#page=N`)
    - normalization: `P16` / `p.16` → `p16`, `p19–20` → `p19-20`
...
The site builder renders this as a visible hyperlink **and** shows `@ <locator>` inline.

## Chapter anchors section

In `manuscript/chapters/*.md`, the `## Anchors (sources + timecodes)` section is used by tooling (`scripts/add_bach_anchors.py`). Anchor bullets should use the list citation form and include a `(keywords: ...)` tail. (Despite the heading name, anchors may use either timecodes or PDF page locators.)
